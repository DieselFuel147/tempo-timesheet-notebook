...
use crate::state::{NotebookBlock, NotebookDay, Settings};
...
mod tests {
    ...
    use crate::state::{NotebookBlock, NotebookDay, Settings, ThresholdSettings};
    ...
    #[test]
    fn mark_synced_updates_notebook_block_metadata() {
        let mut repo = Repository::in_memory().expect("repo");
        repo.save_notebook_day(&NotebookDay {
            date: "2026-07-12".into(),
            blocks: vec![NotebookBlock {
                id: "b1".into(),
                date: "2026-07-12".into(),
        ...
        .expect("save notebook day");

        repo.mark_synced("b1", 42).expect("mark synced");

        let saved = repo.get_notebook_day("2026-07-12").expect("reload notebook day");
        assert_eq!(saved.blocks[0].tempo_worklog_id, Some(42));
        assert!(saved.blocks[0].synced_at.is_some());
    }
