...
import { isPersistedNotebookBlock, isUntrackedBlock, notebookBlockSummary } from '@shared/notebook'
...
export function isPushableBlock(block: NotebookBlock): boolean {
  // UNTRACKED entries never sync: they are visual gap-fillers, not worklogs.
  return !isUntrackedBlock(block) && block.closed && block.startMinute !== null && block.endMinute !== null && notebookBlockSummary(block).trim().length > 0
}
