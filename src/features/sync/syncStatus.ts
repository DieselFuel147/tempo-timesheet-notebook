...
import { isLunchBlock, isPersistedNotebookBlock, notebookBlockSummary } from '@shared/notebook'
...
export function isPushableBlock(block: NotebookBlock): boolean {
  // LUNCH entries never sync: they are visual gap-fillers, not worklogs.
  return !isLunchBlock(block) && block.closed && block.startMinute !== null && block.endMinute !== null && notebookBlockSummary(block).trim().length > 0
}
